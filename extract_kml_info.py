...
def extract_info_and_geocode(kml_file_path):
    ...
    for placemark in placemarks:
        name = placemark.find('kml:name', ns).text if placemark.find(
            'kml:name', ns) is not None else ''
        description = placemark.find('kml:description', ns).text if placemark.find(
            'kml:description', ns) is not None else ''
        coord_text = placemark.find('.//kml:coordinates', ns).text if placemark.find(
            './/kml:coordinates', ns) is not None else ''
        if coord_text:
        ...
        data.append(
            {'Nombre': name, 'Dirección': address, 'Notas': description})
